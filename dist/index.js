...
const yyjlData = {};
const kData = {};
async function readData(filePath, department) {
    ...
    const now = new Date();
    const outPath = path_1.default.resolve(path_1.default.dirname(filePath), `${department}-${now.getMonth()}${now.getDate()}${now.getHours()}${now.getMinutes()}.xlsx`);
    fs_1.default.writeFileSync(outPath, node_xlsx_1.default.build(xlsxData));
    console.log("文件保存在:", outPath);
}
function getEmptyArray(len) {
    return new Array(len).fill(["", "", "", "", "", ""]);
}
function readYYJLData(filePath) {
    try {
        const workSheetsFromFile = node_xlsx_1.default.parse(filePath);
...
async function main() {
    const sourceResult = await inquirer_1.default.prompt({
        type: "input",
        message: "请输入文件地址",
        name: "data",
    });
    ...
    let oldFilePath = oldFileResult.data;
    oldFilePath = oldFilePath.replace(/\'/g, "").replace(/\"/g, "").trim();
    const departmentResult = await inquirer_1.default.prompt({
        type: "input",
